...
void Lab5::Update(float deltaTimeSeconds)
{
    ...
    {
        glm::mat4 modelMatrix = glm::mat4(1);
        modelMatrix = glm::translate(modelMatrix, glm::vec3(-6, 2.0f, 0));
...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_O)) {
            projectionMatrix = glm::ortho(ortholeft, orthoright, orthobottom, orthotop);
        }
        //proiectie persepctiva
        if (window->KeyHold(GLFW_KEY_P)) {
            projectionMatrix = glm::perspective(perspfovy, perspaspect, perspnear, perspfar);
        }
        //modificare pozitiva a FOV
        if (window->KeyHold(GLFW_KEY_J)) {
            perspfovy -= deltaTime;
            projectionMatrix = glm::perspective(perspfovy, perspaspect, perspnear, perspfar);
        }
        //modificare negativa a FOV
        if (window->KeyHold(GLFW_KEY_K)) {
            perspfovy += deltaTime;
            projectionMatrix = glm::perspective(perspfovy, perspaspect, perspnear, perspfar);
        }
        //modificare latime a orto
        if (window->KeyHold(GLFW_KEY_N)) {
            ortholeft -= deltaTime;
            orthoright += deltaTime;
            projectionMatrix = glm::ortho(ortholeft, orthoright, orthobottom, orthotop);
        }
        //modificare inaltime a orto
